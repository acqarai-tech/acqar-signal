...
def _get_rss_feeds() -> dict:
    """Build feed configs with dynamic year at fetch time."""
    ...
    return {
        ...
        "google_news_dubai_re": {
            "url": f"https://news.google.com/rss/search?q=Dubai+real+estate+property+{year}&hl=en-AE&gl=AE&ceid=AE:en",
            "source": "Google News Dubai RE",
            "weight": 0.75
        },
        "google_news_dld_rera": {
            "url": f"https://news.google.com/rss/search?q=Dubai+Land+Department+RERA+real+estate+{year}&hl=en-AE&gl=AE&ceid=AE:en",
            "source": "Google News DLD RERA",
            "weight": 0.8
        }
    }
