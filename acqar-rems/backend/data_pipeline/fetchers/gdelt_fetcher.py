...
Dubai Real Estate News Aggregator - replaces GDELT.
Uses curated Dubai/UAE property news RSS feeds that are reliably Dubai RE focused.
FIXED: Dynamic year in queries, strict date filtering.
"""
import httpx
import asyncio
import hashlib
...
def _get_feeds() -> dict:
    """Build feed URLs dynamically so year is always current at fetch time."""
    ...
    return {
        ...
        "dld_google_news": {
            "url": f"https://news.google.com/rss/search?q=Dubai+Land+Department+DLD+RERA+{year}&hl=en-AE&gl=AE&ceid=AE:en",
            "source": "DLD/RERA News",
            "weight": 0.90,
        },
        "emaar_damac_news": {
            "url": f"https://news.google.com/rss/search?q=Emaar+DAMAC+Nakheel+Dubai+property+launch+{year}&hl=en-AE&gl=AE&ceid=AE:en",
            "source": "Developer News",
            "weight": 0.88,
        },
        "dubai_transaction_news": {
            "url": f"https://news.google.com/rss/search?q=Dubai+property+transaction+AED+villa+apartment+sold+{year}&hl=en-AE&gl=AE&ceid=AE:en",
            "source": "Dubai Transactions",
            "weight": 0.87,
        },
        "palm_marina_downtown": {
            "url": f"https://news.google.com/rss/search?q=Palm+Jumeirah+Dubai+Marina+Downtown+Dubai+real+estate+{year}&hl=en-AE&gl=AE&ceid=AE:en",
            "source": "Prime Areas News",
            "weight": 0.85,
        },
        "dubai_offplan": {
            "url": f"https://news.google.com/rss/search?q=Dubai+off-plan+launch+new+development+handover+{year}&hl=en-AE&gl=AE&ceid=AE:en",
            "source": "Off-Plan News",
            "weight": 0.86,
        },
        "dubai_rental_investment": {
            "url": f"https://news.google.com/rss/search?q=Dubai+rental+yield+investment+ROI+property+market+{year}&hl=en-AE&gl=AE&ceid=AE:en",
            "source": "Investment News",
            "weight": 0.84,
        },
    }
...
# Hard Dubai RE relevance filter
MUST_CONTAIN_ANY = [
    ["dubai", "dld", "rera", "damac", "emaar", "nakheel", "meraas", "dubai land", "bayut", "property finder", "dubailand"],
]
MUST_CONTAIN_PROPERTY = [
...
class GDELTFetcher:
    ...
    async def fetch_dubai_events(self, hours_back: int = 48) -> List[Dict]:
        """Fetch Dubai RE news with dynamic year URLs and date filtering."""
        feeds = _get_feeds()
        tasks = [
            self._fetch_rss_feed(name, cfg)
        ...
        logger.info(f"DubaiNewsAggregator: {len(articles)} fetched → {len(fresh)} after {hours_back}h date filter")
        return fresh
