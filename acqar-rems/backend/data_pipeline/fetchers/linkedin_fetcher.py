...
class LinkedInFetcher:
        ...
        try:
            ...
            for item in channel.findall("item")[:15]:
                ...
                articles.append({
                    "id": hashlib.md5(link.encode()).hexdigest()[:12],
                    "title": title[:200],
                    "summary": desc[:500] if desc and len(desc) > len(title) else title[:200],
                    "url": link,
                    "published_at": published_at,
                    "source": "LinkedIn/Google News",
                    "source_weight": 0.65
        ...
        try:
            ...
            for item in channel.findall("item")[:15]:
                ...
                articles.append({
                    "id": hashlib.md5(url.encode()).hexdigest()[:12],
                    "title": title[:200],
                    "summary": item.get("summary", title)[:500] or title[:200],
                    "url": url,
                    "published_at": published_at,
                    "source": "LinkedIn/GDELT",
                    "source_weight": weight
    ...
    async def fetch_all(self) -> List[Dict]:
        ...
        async with httpx.AsyncClient(headers=self.headers) as client:
            ...
            for q in GDELT_LINKEDIN_QUERIES:
                tasks.append(self._fetch_gdelt_linkedin(client, q))
            # Google News RSS removed — produces broken encoded URLs 
            results = await asyncio.gather(*tasks, return_exceptions=True)
            for r in results:
                if isinstance(r, list):
                    all_articles.extend(r)
