...
class DLDFetcher:
    ...
    async def fetch_area_price_data(self) -> Dict:
        ...
        return result
async def fetch_google_news_transaction_signals(self) -> List[Dict]:
    """Fetch DLD transaction news from direct RSS feeds."""
    import hashlib
    import xml.etree.ElementTree as ET
    import re
    ...
    articles = []
    feeds = [
        ("https://wam.ae/en/rss.xml", "WAM UAE"),
        ("https://www.tradearabia.com/rss/REAL.xml", "Trade Arabia"),
        ("https://www.arabianbusiness.com/rss/topic/real-estate.rss", "Arabian Business"),
        ("https://gulfnews.com/rss/business/property", "Gulf News"),
    ]
    try:
                ...
                try:
                    ...
                    for item in channel.findall("item")[:8]:
                        ...
                        if d_el is not None and d_el.text:
                            desc = re.sub(r'<[^>]+>', '', d_el.text).strip()

                        articles.append({
                            "id": hashlib.md5(l.text.encode()).hexdigest()[:12],
                            "title": t.text[:200],
                            "summary": desc[:500] if desc and len(desc) > len(t.text) else t.text[:200],
                            "url": l.text,
                            "source": "DLD/Google News",
                            "source_weight": 0.8,
                            "published_at": datetime.now(timezone.utc).isoformat()
        ...
        logger.warning(f"Direct RSS fetch failed: {e}")
    return articles
