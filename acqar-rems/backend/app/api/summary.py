...
CACHE_TTL = 86400  # 24 hours
CACHE_RESET_HOUR_UTC = 4  # 4 AM UTC = 8 AM Dubai time (UTC+4)


def _cache_is_valid() -> bool:
    """Check if cache is valid for today after the daily reset hour."""
    if not _cache["content"]:
        return False

    now = datetime.now(timezone.utc)
    generated_at = datetime.fromtimestamp(_cache["generated_at"], tz=timezone.utc)
    generated_date = generated_at.date()
    generated_hour = generated_at.hour

    if now.hour >= CACHE_RESET_HOUR_UTC:
        return (
            generated_date == now.date()
            and generated_hour >= CACHE_RESET_HOUR_UTC
        )
    else:
...
async def get_ai_summary(
    ...
    # ✅ New date-aware cache check replaces old TTL-only check
    if _cache_is_valid():
        age = time.time() - _cache["generated_at"]
        return {
            "summary": _cache["content"],
