...
//                 <div
//                   key={area.area_id}
//                  onClick={() => navigate(`/areas/${area.area_name_en.toLowerCase().replace(/\s+/g, '-')}`)}
//                   className="area-card"
//                   style={{
//                     display: 'flex', alignItems: 'center',
//                     justifyContent: 'space-between',
...
//   useEffect(() => {
//     fetch(
// `${SUPA_URL}/rest/v1/area_intelligence?select=area_id,area_name_en,investment_score,gross_yield_pct,truvalu_psm,distress_pct,tx_7d,verdict,ranking_score,ranking_rank&order=area_name_en.asc&limit=500`,
//       { headers: { apikey: SUPA_KEY, Authorization: `Bearer ${SUPA_KEY}` } }
//     )
//       .then(r => r.json())
//       .then(data => { setAreas(data || []); setLoading(false) })
...
//     }}>
//       {Number(area.ranking_score).toFixed(1)}<span style={{ fontSize: 9, fontWeight: 600, color: C.muted }}>/10</span>
//     </span>
//   )}
...
//   if (e.target.value.length > 2) trackEvent('area_searched', { query: e.target.value, page: 'all_areas' })
// }}
//             placeholder="Search areas..."
//             style={{
//               width: '100%',
...
//   navigate(`/areas/${area.area_name_en.toLowerCase().replace(/\s+/g, '')}`)
// }}
//                   className="area-card"
//                   style={{
//                     display: 'flex', alignItems: 'center',
...
// if (urlUsername) sessionStorage.setItem('acqar_username', urlUsername)
// const username = urlUsername || sessionStorage.getItem('acqar_username') || ''
// if (urlUsername) window.history.replaceState({}, '', window.location.pathname)
// const slugFromUrl = window.location.pathname.split('/areas/')[1]?.replace(/\//g, '') || ''
// const [areas, setAreas] = useState([])
//   const [loading, setLoading] = useState(true)
//   const [search, setSearch] = useState('')
...
//   if (slugFromUrl) {
//     const match = data?.find(a => {
//       const s = a.area_name_en.toLowerCase().replace(/\s+/g, '-')
// .replace(/[^a-z0-9-]/g, '')
...
<div style={{
  ...
  {/* Right: verdict legend + close button */}
  <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
    ...
    <button
      ...
      onMouseEnter={e => {
        e.currentTarget.style.background = C.redL
