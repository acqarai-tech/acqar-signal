...
function AreaPage() {
  ...
  useEffect(() => {
    ...
    // Prefer id from query param (shareable link), fall back to slug matching
    const idParam = searchParams.get('id')
    const nameParam = searchParams.get('area')
    const zoneParam = searchParams.get('zone')

    // If we have all params from the shareable link, skip the fetch
    if (idParam && nameParam && zoneParam) {
      setArea({
        name: nameParam,
        zone: zoneParam,
        area_id: idParam,
    ...
    fetch(
      ...
      .then(data => {
        const match = data.find(a =>
          a.area_name_en.toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '') === slug
        )
        if (!match) return
        const score = Number(match.investment_score) || 60
        setArea({
  ...
  if (!area) return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100vh', fontFamily: 'Inter, sans-serif', color: '#6E7A8A' }}>
      Loading...
    </div>
  )
