...
export default function FilterBar() {
  ...
  return (
   <div style={{borderBottom:'1px solid var(--border-color)', padding:'8px', background:'var(--tab-bar-bg)', flexShrink:0}}>
      {/* Time filter */}
      <div style={{display:'flex', gap:'4px', marginBottom:'8px'}}>
        {[
          {label:'1H', hours:1},
          ...
          <button key={label}
            ...
            style={{
              flex:1, padding:'4px', fontSize:'11px', fontWeight:600,
              border:'1px solid ' + (filters.hours === hours ? '#B87333' : 'var(--border-color)'),
background: filters.hours === hours ? 'rgba(184,115,51,0.2)' : 'transparent',
color: filters.hours === hours ? '#B87333' : 'var(--text-muted)',
              borderRadius:'4px', cursor:'pointer'
            }}
          >{label}</button>
        ))}
