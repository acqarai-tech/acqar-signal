...
//     <>
//       <style>{`
//         .acq-footer-new { position:relative; background:#F5F5F4; border-top:1px solid rgba(10,10,10,0.06); font-family:'Inter',sans-serif; flex-shrink:0; }
...
//         .acq-footer-new ul li.muted { cursor:default; }
...
//             </div>
//             <div>
//               <div className="col-heading"><span className="col-heading-dot" /><h6>Product</h6></div>
//               <ul>
...
//             </div>
//             <div>
//               <div className="col-heading"><span className="col-heading-dot" /><h6>Company</h6></div>
//               <ul>
...
//             </div>
//             <div>
//               <div className="col-heading"><span className="col-heading-dot" /><h6>Legal & Info</h6></div>
//               <ul>
...
//             </div>
//             <div>
//               <div className="col-heading"><span className="col-heading-dot" /><h6>Comparisons</h6></div>
//               <ul>
...
// // if (!myName) return window.location.href = 'https://www.acqar.com/login'
// if (!myName) return window.location.href = `https://www.acqar.com/login?redirect=${encodeURIComponent(window.location.href)}`
//   if (loading) return
//   setLoading(true)
...
//       cursor: 'pointer', color: isReplying ? C.orange : C.muted,
...
//   fetch(
//     `${SUPA_URL}/rest/v1/area_intelligence?area_id=eq.${area.area_id}&select=*`,
//     { headers: { apikey: SUPA_KEY, Authorization: `Bearer ${SUPA_KEY}` } }
//   )
//     .then(r => r.json())
//  .then(data => {
...
//   const SUPA_KEY = import.meta.env.VITE_SUPABASE_ANON_KEY
//   fetch(
//     `${SUPA_URL}/rest/v1/avm?area_id=eq.${area.area_id}&select=price_per_sqm,procedure_area,rooms_en&limit=5000`,
//     { headers: { apikey: SUPA_KEY, Authorization: `Bearer ${SUPA_KEY}` } }
//   )
//     .then(r => r.json())
//     .then(rows => {
...
// const DLD_NAME_MAP = {
//   // Frontend name → dld_projects name
...
//   'Dubai Production City (IMPZ)':    'Me\'Aisem Second',
...
//   'Al Yelayiss 5':                   'Al Yelayiss 5',
//   'Bukadra':                         'Bukadra',
//   'Saih Shuaib 2':                   'Saih Shuaib 2',
//   'Trade Center First':              'Trade Center First',
//   'Trade Center Second':             'Trade Center Second',
//   'Jabal Ali Industrial Second':     'Jabal Ali Industrial Second',
...
//   'Al Barshaa South Third':          'Al Barshaa South Third',
//   'Nad Al Shiba First':              'Nad Al Shiba First',
//   'Jabal Ali First':                 'Jabal Ali First',
//   'Me\'Aisem First':                 'Me\'Aisem First',
//   'Me\'Aisem Second':                'Me\'Aisem Second',
//   'Saih Shuaib 1':                   'Saih Shuaib 1',
// }
...
//   fetch(
//     `${SUPA_URL}/rest/v1/avm?area_id=eq.${area.area_id}&sale_year=gte.2024&select=property_sub_type_en,property_usage_en,rooms_en&limit=10000`,
//     { headers: { apikey: SUPA_KEY, Authorization: `Bearer ${SUPA_KEY}` } }
//   )
//     .then(r => r.json())
//     .then(rows => {
...
//   const SUPA_KEY = import.meta.env.VITE_SUPABASE_ANON_KEY
//   fetch(
//    `${SUPA_URL}/rest/v1/tx_volume_monthly?area_id=eq.${area.area_id}&sale_year=gte.2025&select=sale_year,sale_month,tx_count&order=sale_year.asc,sale_month.asc`,
//     { headers: { apikey: SUPA_KEY, Authorization: `Bearer ${SUPA_KEY}` } }
//   )
//     .then(r => r.json())
//     .then(rows => {
...
//   askGroq(`You are a Dubai real estate research AI. List the latest confirmed infrastructure projects coming to ${area.name} Dubai in 2025-2028: metro stations, schools, hospitals, malls, roads. Return ONLY a valid JSON array, no markdown, no explanation, no backticks. Max 5 items. Format: [{"name":"...","date":"Q4 2026","type":"metro","confidence":"confirmed","impact":"+8-14% PSF"}]. Only include real officially announced projects.`, 600)
//     .then(text => {
//       if (!text) return
//       try {
...
//   onClick={() => window.location.href = 'https://www.acqar.com/'}
// >
...
//     if (names?.length > 0) return `incl. ${names.slice(0,2).join(', ')}${names.length > 2 ? '…' : ''}`
//     return `Active off-plan projects in ${area.name}${units}${isEst ? ' (est.)' : ''}`
//   })(),
// },
//             ].map(m => (
...
//         <div style={{ fontSize: 10, color: C.muted2, textAlign: 'right', marginBottom: 4, fontStyle: 'italic' }}>
//           Estimated trend — live data not yet available for this area
//         </div>
//       )}
//       <PriceHistoryChart data={priceHistory.length > 0 ? priceHistory : (() => {
//         const basePsf = livePsf || 1200
//         const startPsf = Math.round(basePsf / 1.6)
//         return Array.from({ length: 60 }, (_, i) => {
//           const year = 2020 + Math.floor(i / 12)
//           const month = (i % 12) + 1
//           const growth = 1 + (i / 60) * 0.6 + (Math.sin(i / 3.5) * 0.025)
//           return { key: `${year}-${month}`, psf: Math.round(startPsf * growth), year, month, count: 0 }
//         })
//       })()} />
//     </>
//   )}
...
//     <CardTitle badge={devTrackRecords ? 'Market Research ' : 'Historical estimates'}>
...
//     <>
//       <TxVolumeChart
//         data={txHistory?.length > 0 ? txHistory : (() => {
//           const months = ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec']
//           const base = Math.max(10, Math.round(20 + liveScore * 1.2))
//           return Array.from({ length: 12 }, (_, i) => {
//             const d = new Date(2025, new Date().getMonth() - 11 + i + 12)
//             const yr = d.getFullYear(), mo = d.getMonth() + 1
//             const shock = yr === 2026 && mo >= 4
//             return {
//               key: `${yr}-${mo}`,
//               label: `${months[mo-1]} ${String(yr).slice(2)}`,
//               count: shock ? Math.round(base * 0.45) : Math.round(base * (0.75 + Math.sin(i * 0.8) * 0.25)),
//               year: yr, month: mo,
//             }
//           })
//         })()}
//         currentTx={liveSoldThisWeek}
//       />
//       <p style={{ fontSize: 10, color: C.muted, marginTop: 8 }}>
//         {txHistory?.length > 0
//           ? '📊 DLD registered transactions · Monthly aggregated'
//           : '📊 Estimated volume · Live data pending for this area'}
//       </p>
//     </>
//   )}
// </Card>
...
//     park:     'Green space and community amenity improving resident satisfaction and occupancy rates.',
//   }[ev.catalyst_type] ?? 'Infrastructure catalyst confirmed by official sources.'

//   const desc = ev.description ?? fallbackDesc   // ← use DB description if exists
//       const typeImpact = {
...
//       </style></head><body>
//       <div style="display:flex;align-items:center;justify-content:space-between;padding-bottom:16px;border-bottom:2px solid #C8732A;margin-bottom:24px;">
//   <div style="display:flex;align-items:center;gap:10px;">
//     <span style="font-weight:900;font-size:20px;letter-spacing:0.12em;">
//       <span style="color:#C8732A;">ACQ</span><span style="color:#1C1C28;">AR</span>
//     </span>
...
//       <div class="footer" style="margin-top:40px;padding-top:20px;border-top:1px solid #E8E0D0;">
//   <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:12px;margin-bottom:10px;">
//     <div style="display:flex;align-items:center;gap:10px;">
//       <span style="font-weight:900;font-size:16px;letter-spacing:0.12em;">
//         <span style="color:#C8732A;">ACQ</span><span style="color:#1C1C28;">AR</span>
//       </span>
//       <span style="font-size:9px;font-weight:700;color:#C8732A;letter-spacing:1.5px;text-transform:uppercase;padding:2px 8px;border-radius:4px;background:rgba(200,115,42,0.09);border:1px solid rgba(200,115,42,0.35);">SIGNAL™</span>
//       <span style="width:1px;height:12px;background:#E8E0D0;display:inline-block;"></span>
//       <span style="font-size:9px;color:#9CA8B4;font-weight:600;">Dubai, United Arab Emirates</span>
//     </div>
//     <span style="font-size:9px;color:#9CA8B4;">© 2026 ACQARLABS L.L.C-FZ. All rights reserved.</span>
//     <span style="font-size:9px;color:#9CA8B4;">Not financial advice.</span>
//   </div>
//   <div style="font-size:8px;color:#C8CAD0;">Generated ${new Date().toISOString()} · Source: Dubai Land Department · Truvalu™ RICS-aligned valuation · acqar.com</div>
// </div>
//       </body></html>
//     `
...
// ── PIPE CARD ──────────────────────────────────────────────────────
function PipeCard({ dev, name, delivery, units, psfFrom, sold, builtPct, status, hideDev }) {
  const stMap = { ontime: { bg: C.greenL, color: C.green, label: 'On Schedule' }, delayed: { bg: C.redL, color: C.red }, ahead: { bg: C.blueL, color: C.blue, label: 'Ahead of Schedule' } }
  const st = stMap[status] || stMap.ontime
  const soldColor = sold >= 80 ? C.green : sold >= 60 ? C.amber : C.red
  return (
    <div style={{ background: C.card2, border: `1px solid ${C.border}`, borderRadius: 8, padding: 14 }}>
      {!hideDev && <div style={{ fontSize: 10, color: C.muted, textTransform: 'uppercase', letterSpacing: '.08em', marginBottom: 3 }}>{dev}</div>}
      <div style={{ fontSize: 13, fontWeight: 700, color: C.text, marginBottom: 10 }}>{name}</div>
      {[['Delivery', delivery], ['Units', units], ['PSF from', psfFrom], ['Sold', `${sold}%`]].map(([k, v]) => (
        <div key={k} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, marginBottom: 4 }}>
          <span style={{ color: C.muted }}>{k}</span>
