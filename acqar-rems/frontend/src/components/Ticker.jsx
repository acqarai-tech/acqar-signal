...
export default function Ticker() {
  ...
  return (
   <div style={{
  height:'32px', background:'var(--tab-bar-bg)', borderBottom:'1px solid var(--border-color)',
  overflow:'hidden', flexShrink:0, position:'relative'
}}>
      {/* Label */}
      <div style={{
      ...
      <div style={{
        ...
        >
          {items.map((t, i) => (
            <div key={i} style={{display:'flex', alignItems:'center', gap:'6px', flexShrink:0}}>
              <span style={{fontSize:'11px', color:'var(--text-muted)'}}>{t.name || t.symbol}</span>
              ...
              <span style={{color:'var(--border-color)', marginLeft:'8px'}}>|</span>
            </div>
          ))}
        </div>
      </div>
