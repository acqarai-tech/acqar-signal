...
export default function AreaListDrawer({ onClose, onSelectArea }) {
  ...
  useEffect(() => {
    ...
    // Fetch 1: area momentum (event counts per area)
    ...
    fetch(`${API_BASE}/api/market/area-prices`)
      ...
      .then(data => {
        ...
        Object.entries(areas).forEach(([name, info]) => {
          map[name] = {
            pricePerSqft: info.price_sqft_aed,
            yield: info.rental_yield_pct,
          }
        })
        setLiveData(prev => ({ ...prev, ...map }))
      })
  ...
  const filtered = useMemo(() => {
    // Merge live API data on top of hardcoded fallbacks
