...
//   // 4. Navigate to login — replace history so back button
...
//   // ── If inside iframe, hide nav but show tickers ──
// if (isEmbedded) {
//   return (
//     <>
//       <style>{NAV_CSS}</style>
...
//   try {
//     // Call your own backend — yfinance runs server-side (CORS-safe)
//     const res = await fetch(`${API_URL}/api/market/predictions`)
//     if (!res.ok) throw new Error('Backend unavailable')
//     const data = await res.json()
//     if (data.predictions?.length > 0) return data.predictions
//   } catch (_) {}

//   // Fallback: return static placeholders so ticker always renders
//   return UAE_RE_STOCKS.map(s => ({
//     id:       s.symbol,
//     question: `${s.name} (${s.exchange})`,
//     pct:      null,
//     price:    null,
//     change:   null,
//     source:   s.exchange,
...
//               animation: isRealData ? 'hdrLivePulse 2s infinite' : 'none',
//             }} />
//             {isMobile ? (isRealData ? 'DFM' : 'UAE') : sourceLabel}
//           </div>
//           <div style={{ flex: 1, overflow: 'hidden', position: 'relative', height: '100%' }}>
//             {predLoading ? (
//               <div style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, display: 'flex', alignItems: 'center', paddingLeft: '12px', fontSize: '10px', color: '#555' }}>
...
// function PredictionItem({ prediction, isMobile }) {
//   const { question, pct, price, change, source, url, is_real, type } = prediction

//   // pct here = daily change % (positive/negative)
//   const isStock = type === 'stock'
//   const changeVal = pct ?? change ?? 0
//   const color = changeVal > 0 ? '#27AE60' : changeVal < 0 ? '#E74C3C' : '#F39C12'
//   const sign = changeVal > 0 ? '+' : ''
//   const maxLen = isMobile ? 20 : 22
//   const short = question.length > maxLen ? question.slice(0, maxLen - 3) + '...' : question
...
//       {/* Exchange badge */}
//       <span style={{
//         fontSize: '7px', fontWeight: 800, color: '#B87333',
//         padding: '1px 3px', border: '1px solid #B8733344',
...
//         color: '#DEDEDE', letterSpacing: '0.1px', fontWeight: 600,
//       }}>
//         {short}
//       </span>
...
//           color: '#FAFAFA', fontVariantNumeric: 'tabular-nums',
//         }}>
//           {price.toFixed(2)}
//         </span>
//       )}
...
//       {/* Change % */}
//       {changeVal !== 0 && changeVal != null && (
//         <span style={{
//           fontSize: isMobile ? '9px' : '10px', fontWeight: 700,
//           color, fontVariantNumeric: 'tabular-nums',
//         }}>
//           {sign}{Number(changeVal).toFixed(2)}%
//         </span>
//       )}
...
//       {/* Company name */}
//      <span style={{ fontSize: isMobile ? '8px' : '9px', color: '#B3B3B3', letterSpacing: '0px', whiteSpace: 'nowrap' }}>{short}</span>
...
const NAV_CSS = `
 ...
 .hdr-navLink:hover { color: var(--text-primary); }
...
background: var(--bg-primary);
  ...
  .hdr-menuItem:hover { background: var(--row-hover); }
  ...
  .hdr-menuDivider { height: 1px; background: var(--border-color); margin: 8px 0; }
  ...
  .hdr-menuSignout:hover { background: rgba(239,68,68,0.08); }
  .hdr-menuSignout svg { width: 16px; height: 16px; color: #FF4D4D; }
...
onMouseLeave={e => e.currentTarget.style.background = 'transparent'}
    >
      ...
      {/* Company name */}
    <span style={{ fontSize: isMobile ? '8px' : '9px', color: 'var(--text-muted)', letterSpacing: '0px', whiteSpace: 'nowrap' }}>{short}</span>
      ...
      <span style={{ color: 'var(--border-color)', fontSize: '10px', marginLeft: '4px' }}>◆</span>
    </a>
  )
}
